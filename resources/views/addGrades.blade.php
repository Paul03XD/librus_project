...
<div class="container">
    ...
    @if (Auth::user()->type=='admin' || Auth::user()->type=='teacher')
        <form class="d-flex mb-3" method="post" action="{{route('createGrade',$name)}}">
            @csrf
            <p>Wybierz ocenę</p>
...
<?php
        ...
        $wynik = DB::select("SELECT `users`.`id` as `users_id`,`users`.`name` as `users_name` FROM `users` JOIN `classes` ON `users`.`class_id`= `classes`.`id` WHERE `classes`.`name`=\"$name\"");
